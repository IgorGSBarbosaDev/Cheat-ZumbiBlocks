...
Não existem patches Harmony, escrita em campos do jogo, interceptação de mensagens, alteração de saves ou experimento ativo no plugin. `ILabExperiment`, `ExperimentCoordinator` e `LabModeGuard` são contratos sem integração ao runtime nesta etapa.
